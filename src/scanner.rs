...
pub fn scan_apk(apk_path: &Path, github_token: Option<&String>) -> Result<AppRiskProfile> {
    // 1. Load configs
    let sdks_json =
        std::fs::read_to_string("sdks.json").context("Failed to read sdks.json seed file")?;
    let config: SdksConfig =
        serde_json::from_str(&sdks_json).context("Failed to parse sdks.json")?;
    // ponytail: avoid loading toml dynamically for static config weights. Upgrade path: add serde_json / toml if dynamic user config is really needed.
    ...
    for sdk_name in found_sdks {
        ...
        profile.permission_creep_flags =
            crate::permissions::check_scope_creep(&sdk_def.category, &app_permissions);
        ...
        for res in &elf_triage_results {
            ...
            if name_lower.contains(&vendor_lower)
                || name_lower.contains(&sdk_def.name.to_lowercase())
            {
                profile
                    .suspicious_binary_imports
                    .extend(res.suspicious_imports.clone());
                profile
                    .packed_binaries
                    .extend(res.high_entropy_sections.clone());

                let urls_set = res.extracted_urls.iter().cloned().collect();
                let ips_set = res.extracted_ips.iter().cloned().collect();
                let flagged =
                    crate::network::check_against_blocklist(&urls_set, &ips_set, &blocklist);
    ...
    let flagged_global =
        crate::network::check_against_blocklist(&global_urls, &global_ips, &blocklist);
